	...
	factory: (require) => {
		...
		const DSH_PRESETS = [
			...
			},
			{
				id: "code",
				name: "PTC 模式",
				description: "通过 Code Mode SDK 组合多步操作。"
			},
			{
				id: "minimal",
				name: "极简模式",
				description: "仅提供持久 Bash 和文件编辑工具。"
			},
			{
				id: "cordis",
				name: "创造模式",
		...
		function HarnessPicker({ sessionId, session, clearComposerBlock }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				style: wrap,
				title: locked && !snapshot.blank ? "会话开始后 Harness 不能再切换" : "选择本会话的执行引擎",
		...
		function HeroAgentPresetSeat({ sessions }) {
			...
			const label = DSH_PRESETS.find((preset) => preset.id === snapshot.preset)?.name ?? snapshot.preset;
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
					...
					children: DSH_PRESETS.map((preset) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
						type: "button",
						role: "menuitem",
		...
		function GrokModelSeat({ sessionId, locked, snapshot, onChange }) {
			const [open, setOpen] = (0, react.useState)(false);
			const [saving, setSaving] = (0, react.useState)(false);
			const rootRef = (0, react.useRef)(null);
		...
		function DshModelSeat({ locked, directory, load, select }) {
			...
			const label = state.groups.flatMap((group) => group.models.map((model) => ({
				group,
				model
			}))).find((row) => row.group.id === state.current?.provider && row.model.id === state.current?.model)?.model.name ?? state.current?.model ?? "选择模型";
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				style: {
					...wrap,
					justifyContent: "flex-end"
