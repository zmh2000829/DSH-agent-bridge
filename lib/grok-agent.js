...
export class GrokWebAgent {
  ...
  attachHost(host, remoteSessionId) {
    this.host = host
  ...
  async runWebCommand(name, rawInput) {
    ...
    if (host === undefined || remoteSessionId === undefined) {
    ...
    if (name === 'model' || name === 'effort') {
      ...
      try {
        ...
        if (name === 'model') this.grokModel = value
  ...
  async drive(signal) {
    while (this.inbox.nextTurn.length > 0) {
      ...
      try {
        ...
        if (this.remoteSessionId === undefined || this.host === undefined) {
        ...
        if (this.active !== undefined) {
          this.closeAssistantBlocks(this.active)
  ...
  handleUpdate(params) {
    ...
    if (update.sessionUpdate === 'agent_message_chunk' && update.content?.type === 'text') {
      ...
      if (text.length > 0) this.appendTextDelta(active, 'text', `\n\n${text}\n`)
    }
  ...
  ensureToolCall(active, update) {
    ...
    active.toolNames.set(id, update.title ?? update.kind ?? 'tool')
  ...
  appendToolResult(active, wireId, text, isError, diffs) {
    if (!active.toolNames.delete(wireId)) return
